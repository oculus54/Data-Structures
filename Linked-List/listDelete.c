...
void delete() //normal loop process
{
    Node *current = head; //saves the current node
    while(current){
        Node *temp = current->next;//temporary pointer created for saving the next pointer's address
        free(current);
        current = temp;//switch current to next node
    }
    head = NULL;    //set head to null to avoid dangling pointer issue
}